...
#  The "extra_clusters_added" field kinda screws some things up. So I will read in genotyped sampled numbers via two queries.
#  One that includes the field and one that avoids it. 

genos_sans <- read_excel("rcode/Data/Successfully genotyped.xlsx",sheet="n=3491 All Scores & Binary") %>% 
  rename_all(tolower) %>% 
...
folder.names <- list.files(path="rcode/Data/bayes/spatial",pattern=".")
myout <- data.frame()

for(i in 1:length(folder.names)){
  print(folder.names[i])
  myrgn <- list.files(path=paste0("rcode/Data/bayes/spatial/",folder.names[i]),pattern="RGN")
  
  test <- lapply(myrgn,function(x)data.frame(chain=x,read.table(paste0("rcode/Data/bayes/spatial/",folder.names[i],"/",x))))
...
for(i in 1:length(folder.names)){
  print(i)
  myrgn <- list.files(path=paste0("rcode/Data/bayes/spatial/",folder.names[i]),pattern="estimate")
  
  shrink <- read.table(paste0("rcode/Data/bayes/spatial/",folder.names[i],"/",myrgn),skip=450,nrows = 6) %>% 
    bind_cols(data.frame(Region)) %>% 
    dplyr::select(Region,`Shrink Factor`=V4)
  
  comp <- read.table(paste0("rcode/Data/bayes/spatial/",folder.names[i],"/",myrgn),skip=539,nrows = 6) %>% 
    bind_cols(data.frame(Region)) %>%   
...
mygroupings <- c("GOA","B-season","BS Early","BS Middle","BS Late","EBS","WBS")
...
png("Figures/figure4.png",width=6.5,height=6.5,units="in",res=300)
grid.newpage()
...
labelsequence <- seq(0,max(goa$Chum),by=1000)
...
# No threshold / "other" group (below)
...
goa.c.start2 <- format(as.POSIXct(strptime("08/25/2017","%m/%d/%Y")),"%m/%d")
goa.c.end2 <- format(as.POSIXct(strptime("09/30/2017","%m/%d/%Y")),"%m/%d")
goa.d.start2 <- format(as.POSIXct(strptime("10/01/2017","%m/%d/%Y")),"%m/%d")
goa.d.end2 <- format(as.POSIXct(strptime("10/31/2017","%m/%d/%Y")),"%m/%d")
